const sortedSpot = spots.sort(() => { return (Math.random() - Math.random()) }).slice(0,3)
var loc;
var grub;
...
window.onload = () => {
 for (i = 0; i < sortedSpot.length; i++) {
   displaySearchResults(onLoaSpot, i);
 }
};
...
function reply_click() {
  ...
  for (x=0; x<sortedSpot.length; x++){
    if(sortedSpot[x].uId === id){
      clickedSpot.push(sortedSpot[x]);
    }
  }
