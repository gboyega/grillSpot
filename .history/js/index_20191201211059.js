...
form.addEventListener("submit", event => {
  ...
  if (loc != "Location" || grub != "Looking for") {
    if (loc != "Location" && grub === "Looking for") {
      indexContent.innerHTML = `<h3 class="text-success text-center">${loc}</h3>`;
    } else if (loc === "Location" && grub != "Looking for"){
      indexContent.innerHTML = `<h3 class="text-success text-center">${grub}</h3>`;
    } else {indexContent.innerHTML = `<h3 class="text-success text-center">${grub} in ${loc}</h3>`;};          
